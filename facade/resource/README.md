...
> **Status**: 전략별 설정 파일(`detr.yaml`, `dots_mocr.yaml`, `rule.yaml`, `paddle.yaml`)은 스키마
> 정의 단계입니다. 현재 코드(`BaseLoader.get_layout()`)는 `layout.type` 값과 무관하게 항상
> rule 기반 `Layout`을 반환하며, `config: /app/resource/<이름>.yaml`을 실제로 읽어와 분기하는
> 로직은 아직 연결되지 않았습니다.
...
### `rule` — 폰트 크기 휴리스틱 (`util/layout.py`)
...
> **Planned** (아직 스키마에 없음, 실제로 서빙을 붙일 때 추가 검토): `model_name`
> (`docling_layout_heron` / `egret_medium` / `egret_large` / `egret_xlarge`),
> `confidence_threshold`(기본 `0.3`), `device`(`cpu`/`cuda`/`mps`), `image_dpi`(페이지 렌더링
> 해상도), `timeout`, `blacklist_classes`.
