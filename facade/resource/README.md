...
> **Status**: `rule`, `detr`, `dots_mocr` 모두 실제로 연결되어 있습니다. `layout.type`이
> `rule`이 아니면 `DocumentProcessor`가 `resource/<type>.yaml`을 읽어 `config.yaml`의 `layout`
> 설정과 병합하고, `BaseLoader.get_layout()`이 그 값에 따라 `Layout`(rule), `DetrLayout`(detr),
> `DotsMocrLayout`(dots_mocr) 중 하나를 반환합니다.
...
### `dots_mocr` — VLM 프롬프트 기반 원격 레이아웃 서버 (`resource/dots_mocr.yaml`, `util/dots_mocr_layout.py`)

[dots.ocr/dots.mocr](https://github.com/rednote-hilab/dots.ocr)(3B, Qwen2.5-VL 기반)를 vLLM 등으로
서빙한 OpenAI 호환 chat completions 엔드포인트를 호출한다. 페이지 이미지 + 프롬프트를 보내면
모델이 `[{"bbox": [x1, y1, x2, y2], "category": "...", "text": "..."}, ...]` 형태의 JSON 배열을
텍스트로 돌려주는데, detr와 동일한 방식으로 이 region의 bbox/category만 써서 로더가 이미 추출한
줄의 category를 매긴다 — region의 `text`(모델이 직접 인식한 문자)는 쓰지 않는다.

`genonai/doc_parser`(GenOS의 기존 전처리기)가 실제 프로덕션에서 dots-mocr를 GenOS llmops
게이트웨이로 서빙해 호출하는 `docling/models/genos_dots_ocr_layout_model.py`를 참고해 만들었다.
그쪽은 텍스트까지 포함하는 `prompt_layout_all_en`을 기본으로 쓰고 read timeout이나
`finish_reason == "length"`(텍스트 생성이 토큰 상한까지 폭주하는 degeneration) 감지 시에만
텍스트를 안 뽑는 `prompt_layout_only_en`으로 재시도하는데, 우리는 region의 text 자체를 안
쓰므로 이 fallback용 프롬프트를 처음부터 기본값(`DEFAULT_PROMPT`)으로 써버린다 — 텍스트
생성이 아예 없으니 저 폭주 실패 모드도 원천적으로 없다.

| Key | Type | Default | Description |
|---|---|---|---|
| `url` | str | `null` | chat completions 엔드포인트. 필수. |
| `api_key` | str | `null` | 설정하면 `Authorization: Bearer <api_key>` 헤더로 보낸다. |
| `model` | str | `null` | 요청 payload의 `model` 필드. 서버가 모델 이름으로 라우팅하면 설정(GenOS 게이트웨이 기준 `"dots-mocr"`). |
| `batch_size` | int | `64` | 아직 미사용 — detr와 동일하게 현재는 페이지당 1장씩 호출한다. |
| `timeout` | int | `60` | 호출 타임아웃(초). |
| `prompt` | str | `null` | VLM에 넘길 레이아웃 분석 프롬프트. `null`이면 dots.ocr 공식 `prompt_layout_only_en`(`DEFAULT_PROMPT`)을 쓴다. |
| `max_completion_tokens` | int | `16384` | 모델 응답 최대 토큰 수. `400` 응답이 오면 `max_tokens` 파라미터명으로 한 번 더 시도한다(서버마다 파라미터명이 달라서). |
| `temperature` | float | `0.1` | 생성 temperature. |
| `top_p` | float | `0.9` | 생성 top_p. |
| `repetition_penalty` | float \| null | `1.15` | 반복(degeneration) 억제 페널티. `null`이면 payload에서 뺀다(모든 서버가 지원하지는 않음). |
| `image_token_prefix` | str | `""` | 프롬프트 앞에 붙일 접두어. 표준 OpenAI 호환 서버는 채팅 템플릿이 이미지 placeholder를 자동으로 넣어주므로 빈 문자열이면 된다. GenOS llmops 게이트웨이처럼 채팅 템플릿을 안 태우는 서버는 `genonai/doc_parser` 기준 `"<\|img\|><\|imgpad\|><\|endofimg\|>"` 로 설정. |

dots.ocr는 Qwen2.5-VL 비전 인코더를 쓰기 때문에, 전처리 단계에서 이미지를 28의 배수
해상도로 내부 리사이즈("smart resize")한 뒤 본다. 모델이 돌려주는 bbox는 이 리사이즈된
해상도 기준이라, `_smart_resize`로 그 해상도를 재계산해 우리가 보낸 이미지 픽셀 좌표로
되돌린 다음, 다시 72dpi PDF 포인트로 스케일한다(이 보정을 빠뜨리면 페이지 크기에 따라
카테고리 매칭이 체계적으로 어긋난다). 또한 모델 응답은 토큰 한도로 배열이 중간에 잘리거나
앞뒤에 ` ```json ... ``` ` 코드펜스나 잡담이 섞일 수 있어, 코드펜스를 벗기고 첫 `[` 이전을
버린 뒤 마지막 완전한 `}` 뒤에서 배열을 닫는 식으로 관용적으로 정리한 뒤 파싱한다.
