...
| `prompt` | str | `null` | VLM에 넘길 레이아웃 분석 프롬프트. `null`이면 dots.ocr 공식 레이아웃 프롬프트(`DEFAULT_PROMPT`)를 쓴다. |
| `max_tokens` | int | `24000` | 모델 응답 최대 토큰 수. |

모델 응답은 토큰 한도로 배열이 중간에 잘리거나 앞뒤에 잡담이 섞일 수 있어(dots.ocr 자체의
알려진 특성), `_clean_json_array`가 첫 `[` 이전을 버리고 마지막 완전한 `}` 뒤에서 배열을 닫는
식으로 관용적으로 정리한 뒤 파싱한다.
