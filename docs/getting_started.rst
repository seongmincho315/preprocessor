...
문서 하나가 파일 핸들링 -> 로더(레이아웃/OCR) -> 전처리/보강 -> 청킹 ->
후처리/보강 -> 메타데이터, 8단계를 거쳐 GenOS 서빙용 벡터 dict 목록이 된다.
로더/청커는 ``resource/config.yaml``\ 로 구현체를 갈아 끼울 수 있다:
...
- **전처리**/**pre-Enrichment** - 룰 기반 전처리/외부 모델 보강. 기본은
  항등 함수(스텁).
- **청킹** (``chunker.type``\ ) - ``smart_chunker``\ (기본)/
  ``hierarchical_chunker``\ /``hybrid_chunker`` 중 하나로 아이템을 청크로
  묶는다.
- **후처리**/**post-Enrichment** - 청크 단위 후처리/외부 모델 보강(예:
  image_description, table_refiner). 기본은 항등 함수(스텁).
