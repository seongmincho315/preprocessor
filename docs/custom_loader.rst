...
.. note::
   위 예제는 조합을 코드에 고정하는 :doc:`custom_preprocessor` 방식이라
   ``resource/config.yaml`` 을 읽지 않는다. ``config.yaml`` 로 계속 갈아
   끼우는 :class:`preprocessor.DocumentProcessor` 를 쓰고 싶다면, 이
   ``DocumentProcessor`` 서브클래스를 따로 만들 필요 없이 ``MyLoader``
   를 ``loader/my_ext/my_loader.py`` 에 ``Loader`` 클래스로 저장하고
   ``ext.my_ext: my_loader`` 만 추가하면 된다 - 자세한 건 :doc:`facade_usage`.
